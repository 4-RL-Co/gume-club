...
- **Sem contador de seguidores, sem ofensiva (streak).** Nada em volta da sua LEITURA é contado: nem quem te segue, nem quem leu o quê. A resenha pode receber um **voto** de quem a leu — não um comentário, e o voto é na resenha, nunca em você: não existe um número de "quantos votos esta pessoa já recebeu" em lugar nenhum do app. Fora isso, a única contagem de gente é quantas pessoas guardaram uma coleção, e ela é sobre curadoria. E sem placar: existe uma **honra** no seu perfil, e **não existe lista de quem leu mais**. Uma lista ordenada por leitura é uma máquina de fazer gente mentir que leu.
