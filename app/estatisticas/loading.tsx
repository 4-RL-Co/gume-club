      ...
      <div className="mt-10 flex flex-col gap-10">
        {Array.from({ length: 3 }).map((_, i) => (
          <section key={i} className="surface p-7 sm:p-8">
            <span className="block h-2.5 w-40 animate-pulse rounded-full bg-[color-mix(in_srgb,var(--color-ink)_5%,transparent)]" />
            <div className="mt-7 flex flex-col gap-3">
              {Array.from({ length: 4 }).map((_, j) => (
                <span key={j} className="block h-2.5 animate-pulse rounded-full bg-[color-mix(in_srgb,var(--color-ink)_4%,transparent)]" />
              ))}
            </div>
          </section>
        ))}
