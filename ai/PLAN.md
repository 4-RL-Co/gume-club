...
- [x] **A resenha aparece inteira no feed, com upvote.** "resenhou" já era um verbo do feed, e a linha só mostrava a frase — quem quisesse ler tinha que caçar a resenha sozinho na página do livro. Agora o corpo (cortado, com "ler resenha inteira") e o botão de upvote vêm junto, buscados em lote e com a visibilidade conferida DE NOVO na leitura (`lib/explore.ts`, `getResenhasPorId`), e não só confiados na cópia que a `activity` guardou no instante em que foi escrita.
- [x] **Criar uma estante inventada também vira uma linha do feed.** `activities.work_id` deixou de ser obrigatório (migration 0068): a linha agora pode falar de uma ESTANTE (`collection_id`) em vez de um livro, com a mesma lei de visibilidade e a mesma cautela de conferir de novo na leitura (`getListasPorId`, `lib/listas.ts`). O card reaproveita `components/lista-card.tsx`, o mesmo que /listas usa.
